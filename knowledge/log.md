...
* **Update**: [AIエージェントの「ハーネス」とは何か・どう育てるか](/tech/ai-agent-harness-basics.md) — 量産プロンプトに需要検証の2ゲートを組み込み受注生産化。入口=需要はブレストでなくログ採掘（需要票: 日時・回数・失った時間、原典照合で捏造防止）、仮説需要は検出器（シャドーモード）先行、不可逆損失のみ深刻度レーンで頻度免除。出口=発火カウンタ標準装備+棚卸し基準（発火ゼロ→撤去候補、誤作動>発火→即時撤去）。受注窓口は既存の failure-review 失敗台帳に接続。
