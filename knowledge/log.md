...
* **Update**: [要件定義書からの DB 設計 — 3述語の検証機構と「生成1回＋検証3層」フロー](/tech/db-design-validation-from-requirements.md) — missing 18 の正体は「項目定義がなく設計できない」＝入力の穴の正しい検出。「列挙できる≠列挙されている」。対処は第3入力 items.md（項目定義台帳）と検証者への配線。抽出元は既存 HTML でよい（form 属性は機械抽出向き）が「画面の項目≠保存する項目」の罠があり、entry: input|display タグで導出疑いを分離、仕分けは「複製か事実か」の原理を再利用。人間の仕事は未確定の事実記入のみ。
