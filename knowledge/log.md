...
* **Update**: [AIエージェントの「ハーネス」とは何か・どう育てるか](/tech/ai-agent-harness-basics.md) — gpt-5.6 Sol Ultra の用途を追記。Ultra は無期限の思想会議ではなく、観測・提案・反証・評価を並列化する「ハーネス投資委員会」に限定。失敗観測→投資判断→最小実験→測定→残す／捨てるの閉ループ、6点の必須出力、証拠・重複・効果・維持コスト・予算による停止条件を定義。
