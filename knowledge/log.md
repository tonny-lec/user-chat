...
* **Update**: [構築モード — 製品完成優先の第3モードとそのプロンプト](/tech/build-mode-prompts.md) — #0 ゲート初回実測（今直す9/構築モード35/回答待ち0/引き渡し1/分類不能30）と #0a 後処理を追記: 回答待ち0は消失疑い（直前まで9件前後 — 説明必須）、分類不能30は語彙不足のサイン（対象外大量発行と同族）— 正体は廃止アプローチの残骸と推定し第5分類「廃棄候補」を新設。今直す9件は差分提案リスト化して人間○×。
