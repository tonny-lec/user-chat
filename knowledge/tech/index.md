...
* [codex 単体作業ハーネスの初手設計 — 最初の1本は観測装置](/tech/codex-standalone-harness-bootstrap.md) - gpt-5.6 GA を機に「環境非依存・codex のみで完結」するハーネスを作る相談。Codex CLI 2026 のハーネス面（ネイティブ subagents・安定版 hooks）を確認し、初手は Stop hook 強制つき失敗観測ループのバンドル完結版を推奨。
...
* [業務システムのUIをAIに「利用者視点」で設計させる方法](/tech/business-system-ui-prompting.md) - 「利用者視点に立って」が効かないのは証拠の欠落。LLMのデザイン事前分布はコンシューマサイトに寄るため、1人1タスク仕様・参照例1枚・ウォークスルー述語で条件付ける処方箋。続編で縮退解（片側制約の悪用）の実例と可変メトリクス表の定石を追記。
