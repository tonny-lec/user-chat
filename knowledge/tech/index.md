...
* [ハーネスのトークン経済学 — 「入れない・留めない・再計算しない」の3機構](/tech/harness-token-economy.md) - 「品質を下げずにトークンを減らすハーネス」の作動原理。コストの正体は サイズ×滞在ターン数×キャッシュ係数 であり、削減機構は境界フィルタ・使い捨てコンテキスト・キャッシュ/永続知識の3つに帰着する。品質と両立する理由は「次の判断を変えないトークン」だけを削るから。打ち手のインパクト順と、サブエージェントが逆にトークンを増やす条件を含む。
