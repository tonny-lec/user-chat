...
* [要件定義・設計・実装を同時に進めるフローの構築方針 — 凍結の単位を時間から空間へ](/tech/concurrent-req-design-impl-flow.md) - 同時進行フローの設計課題は進め方ではなく整合性維持。3文書を3解像度のレイヤーとして扱い、縦スライス単位で凍結し、層間の矛盾検出を機械化（受け入れテスト・依存lint・逆流の正規化）する構成。
