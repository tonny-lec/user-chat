...
* [要件定義・設計・実装を同時に進めるフローの構築方針 — 凍結の単位を時間から空間へ](/tech/concurrent-req-design-impl-flow.md) - 同時進行フローの設計課題は進め方ではなく整合性維持。3文書を3解像度のレイヤーとして扱い、縦スライス単位で凍結し、層間の矛盾検出を機械化する構成。続編で実装解像度（層=権限境界・凍結=パス権限・変更伝播表・双方向トレーサビリティ・赤の原因分類）、続編2で横断非機能レーン（NFR3分類・予算表・ラチェット・創発バックストップ）を具体化。
