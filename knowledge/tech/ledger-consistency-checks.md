...
分解の理由（実測 2026-07-28）: **①トークン** — 大プロンプトの繰り返しは
毎回全典拠を読み直させ消費が跳ねる ②**精度** — 対象が1辺だと逃げ道
（最安ラベル・集計だけ報告）の余地が減る ③**再実行** — ×が出た辺だけ
回し直せる。スコープはできるだけ絞るのが既定。
...
**C7 の×の閉じ方（幽霊参照 — 使う項目だと人間が確認済みの場合）**:
参照の削除で○にするのは最安の逃げ道であり禁ずる。正体は3つ —
表記ゆれ（参照名を実名へ）/ db-design の列不足（DB 差分提案・evidence 必須）/
existing-tables のカタログ不足（実在確認のうえ追記）。

```markdown
C7 で検出された幽霊参照を解消せよ。前提: これらは実際に使う項目である
（人間確認済み）。参照の削除で解消することを禁ずる。
各件を3分類し、処置を提案せよ（勝手に反映するな）:
1. 表記ゆれ: db-design / existing-tables に別名・類似名で実在する
   → 実在する実名を挙げ、process-design 側の参照を実名に直す案。
2. db-design の列不足: 新設計テーブルに必要な列が無い
   → DB 差分提案として `テーブル / 列名 / 型 / evidence（REQ か ITEM）`。
   evidence を挙げられない列は提案せず「要検討」として別掲せよ。
3. existing-tables のカタログ不足: 実DB に実在するがカタログ未記載
   → 実在確認の根拠を添えてカタログ追記案（table / key / 該当列 / usage）。
全件を `参照 / 分類(1|2|3) / 処置案` の○×カードで提示し、この場で
人間の○×を待て。
○×後: 承認分を反映し、C7 を再実行して差分のみ報告せよ。
分類2で列を追加した場合は、追加列について C3（順トレース・削除審問）を
該当分のみ再確認せよ。
報告: 分類内訳（1/2/3/要検討）/ 反映 <n> / C7 再実行 <幽霊 残 n> /
C3 影響 <有|無>
```
